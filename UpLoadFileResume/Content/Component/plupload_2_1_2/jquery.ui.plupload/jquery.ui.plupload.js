...
//            // enalert(1);
//            //文件被分块上传的事件
//            //这里假设文件大小1G，20M一个分块，当前为第一次分块。
//            //当第一个分块上传完成时：file.loaded=20M  第二次为40M

//            //现在我想：当用户出现断电等意外时需要断点续传：假设已经上传了100M
//            //当重新传送文件时，传送到了第一个分块时，我会在服务器自动检测是否有其它分块，检测到5个分块
//            //那么我会在info这个josn数据中返回。
//            //在info里我从服务器返回了已经上传的区块总字节数
//            //up.files[0].loaded=100M;
//            //info.offset=100M;
//            //现在我已经得到了，服务器已经上传的区块，那么第二次上传的时候我就想直接从100M开始
//            //而不是20-40M了。这里我设置了file.loaded=100M，但是不起作用
//            
//            //客户端跳过某些区块应该怎样设置呢?
//		});
